...
# إعداد السجلات - تقليل مستوى سجلات httpx لمنع الزحام
...
async def build_cache(bot):
    ...
    for msg_id in range(1, LIST_MESSAGE_ID + 1):
        try:
            ...
            await asyncio.sleep(0.05) # سرعة معقولة لتجنب الحظر
        except Exception:
            # تجاهل الرسائل غير الموجودة صمتاً لتنظيف الـ Logs
            continue
...
async def handle_registration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not match:
        await update.message.reply_text("❌ تنسيق خاطئ! أرسل: `@يوزر | السيريال` مع الصورة.")
        return
    ...
    await update.message.reply_text("✅ تم الفحص، طلبك قيد المراجعة...")
...
def main():
    ...
    # التعديل هنا: استخدام CAPTION بالحروف الكبيرة
    app.add_handler(MessageHandler(filters.PHOTO & filters.CAPTION, handle_registration))
